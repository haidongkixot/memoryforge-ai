...
export default function LibraryPage() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-3xl font-bold text-[#1f2937]">Game Library</h1>
      ...
      <div className="flex flex-wrap gap-3">
        <input type="text" value={search} onChange={e => setSearch(e.target.value)} placeholder="Search games..."
          className="bg-white border border-[#E5E7EB] rounded-xl px-4 py-2 text-[#1f2937] focus:border-[#6366f1] focus:outline-none flex-1 min-w-[200px]" />
        <select value={category} onChange={e => setCategory(e.target.value)}
          className="bg-white border border-[#E5E7EB] rounded-xl px-4 py-2 text-[#1f2937] focus:border-[#6366f1] focus:outline-none">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {games.map(game => (
          <Link key={game.id} href={`/practice/${game.slug}`}
            className="bg-white border border-[#E5E7EB] hover:border-[#6366f1]/50 rounded-2xl p-6 transition-all group hover:shadow-md">
            <div className="flex items-start justify-between mb-3">
              <span className="text-3xl">{CATEGORY_ICONS[game.category] || '🎮'}</span>
              <span className={`text-xs px-2 py-1 rounded-full font-medium ${
                game.difficulty === 'advanced' ? 'bg-[#F87171]/20 text-red-600' :
                game.difficulty === 'intermediate' ? 'bg-[#FFCE00]/20 text-yellow-600' :
                'bg-[#ABF263]/20 text-green-600'
              }`}>{game.difficulty}</span>
            </div>
            <h3 className="text-lg font-semibold text-[#1f2937] group-hover:text-[#6366f1] transition-colors">{game.name}</h3>
            <p className="text-[#6B7280] text-sm mt-1 line-clamp-2">{game.description}</p>
            <div className="flex flex-wrap gap-1 mt-3">
              {game.benefits.slice(0, 2).map(b => (
                <span key={b} className="text-xs bg-[#EEF2FF] text-[#6366f1] px-2 py-0.5 rounded-full font-medium">{b}</span>
              ))}
            </div>
          </Link>
        ))}
